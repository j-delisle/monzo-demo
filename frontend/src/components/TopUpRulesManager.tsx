...
import { AccountCardsGrid } from "@/components/AccountOverviewCards";

interface TopUpRulesManagerProps {
  accounts: Account[];
}

export function TopUpRulesManager({ accounts }: TopUpRulesManagerProps) {
                  ...
                  >
                    <div>
                      <p className="font-medium">
                        TopUp {formatCurrency(rule.topup_amount)} when balance
                        below {formatCurrency(rule.threshold)}
                      </p>
            ...
            <div className="space-y-3">
              {events.slice(0, 5).map((event) => (
                ...
                >
                  <div>
                    <p className="font-medium">
                      {formatCurrency(event.amount)} TopUp
                    </p>
                    <p className="text-sm text-muted-foreground">
                      Triggered at balance:{" "}
                      {formatCurrency(event.triggered_balance)}
                    </p>
